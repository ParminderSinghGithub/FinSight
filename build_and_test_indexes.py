...
print(f"\n  [text]  Encoding {len(text_texts)} chunk(s) ...")
text_vecs = text_embedder.encode(text_texts, batch_size=16)
print(f"  → shape {text_vecs.shape}")

print(f"\n  [code]  Encoding {len(code_texts)} chunk(s) ...")
code_vecs = code_embedder.encode(code_texts, batch_size=8)
print(f"  → shape {code_vecs.shape}")

print(f"\n  [image] Encoding {len(image_paths)} image(s) ...")
image_vecs = image_embedder.encode(image_paths, batch_size=8)
print(f"  → shape {image_vecs.shape}")
...
for chunk_id, score in text_results:
    ...
    if match:
        ...
        print(f"    [{chunk_id}]  section={match['section']}")
        print(f"              \"{snippet}...\"")
...
for chunk_id, score in code_results:
    ...
    if match:
        ...
        print(f"    [{chunk_id}]  fn={match['function_name']}  type={match['chunk_type']}")
        print(f"              \"{snippet}...\"")

# ── Cross-modal image query (CLIP text → image) ───────────────────────────────
IMAGE_QUERY = "DBSCAN clustering with noise"
print(f"\n  [IMAGE cross-modal query]  \"{IMAGE_QUERY}\"")
